...
for {set i 256} {$i<722} {incr i 15} {
	...
	$ns duplex-link $n([expr ($i+3)]) $n([expr ($i+7)]) 1Mb 10ms FQ
	$ns duplex-link $n([expr ($i+3)]) $n([expr ($i+8)]) 1Mb 10ms FQ

	$ns duplex-link $n([expr ($i+4)]) $n([expr ($i+9)]) 1Mb 10ms FQ
	$ns duplex-link $n([expr ($i+4)]) $n([expr ($i+10)]) 1Mb 10ms FQ

	$ns duplex-link $n([expr ($i+5)]) $n([expr ($i+11)]) 1Mb 10ms FQ
	$ns duplex-link $n([expr ($i+5)]) $n([expr ($i+12)]) 1Mb 10ms FQ

	$ns duplex-link $n([expr ($i+6)]) $n([expr ($i+13)]) 1Mb 10ms FQ
	$ns duplex-link $n([expr ($i+6)]) $n([expr ($i+14)]) 1Mb 10ms FQ
}
...
for {set i 263} {$i<489} {incr i 15} {
	...
	$ns duplex-link $n([expr ($i+3)]) $n($k) 1Mb 10ms FQ
	incr k
	$ns duplex-link $n([expr ($i+4)]) $n($k) 1Mb 10ms FQ
	incr k
	$ns duplex-link $n([expr ($i+4)]) $n($k) 1Mb 10ms FQ
	incr k
	$ns duplex-link $n([expr ($i+5)]) $n($k) 1Mb 10ms FQ
	incr k
	$ns duplex-link $n([expr ($i+5)]) $n($k) 1Mb 10ms FQ
	incr k
	$ns duplex-link $n([expr ($i+6)]) $n($k) 1Mb 10ms FQ
	incr k
	$ns duplex-link $n([expr ($i+6)]) $n($k) 1Mb 10ms FQ
	incr k
	$ns duplex-link $n([expr ($i+7)]) $n($k) 1Mb 10ms FQ
	incr k
	$ns duplex-link $n([expr ($i+7)]) $n($k) 1Mb 10ms FQ
	incr k
}
...
for {set i 503} {$i<729} {incr i 15} {
	...
	$ns duplex-link $n([expr ($i+3)]) $n($k1) 1Mb 10ms FQ
	incr k1 16
	$ns duplex-link $n([expr ($i+4)]) $n($k1) 1Mb 10ms FQ
	incr k1 16
	$ns duplex-link $n([expr ($i+4)]) $n($k1) 1Mb 10ms FQ
	incr k1 16
	$ns duplex-link $n([expr ($i+5)]) $n($k1) 1Mb 10ms FQ
	incr k1 16
	$ns duplex-link $n([expr ($i+5)]) $n($k1) 1Mb 10ms FQ
	incr k1 16
	$ns duplex-link $n([expr ($i+6)]) $n($k1) 1Mb 10ms FQ
	incr k1 16
	$ns duplex-link $n([expr ($i+6)]) $n($k1) 1Mb 10ms FQ
	incr k1 16
	$ns duplex-link $n([expr ($i+7)]) $n($k1) 1Mb 10ms FQ
	incr k1 16
	$ns duplex-link $n([expr ($i+7)]) $n($k1) 1Mb 10ms FQ
	incr k
}
