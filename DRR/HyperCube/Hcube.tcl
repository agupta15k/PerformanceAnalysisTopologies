...
for {set j 0} {$j < $as} {incr j} {
	...
	for {set i 0} {$i < 3} {incr i} {
		$ns duplex-link $n($c) $n([expr ($c+1)]) 1Mb 10ms DRR
		incr c
	}
	$ns duplex-link $n($c) $n($c1) 1Mb 10ms DRR
	...
	for {set i 0} {$i < 4} {incr i} {
		$ns duplex-link $n($c) $n([expr ($c+4)]) 1Mb 10ms DRR
		incr c
	}
}
...
for {set j 0} {$j < $as-1} {incr j} {
	for {set i [expr ($j*8)]} {$i < [expr (($j*8)+8)]} {incr i 1} {
		$ns duplex-link $n($i) $n([expr ($i+8)]) 1Mb 10ms DRR
	}
}
